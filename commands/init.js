...
export async function init() {
  ...
  const listAwsProfiles = execSync('aws configure list-profiles', {
    encoding: 'utf8',
  })
    .split('\n')
  ...
  const selectAwsProfile = await select({
    ...
    choices: listAwsProfiles.map((profile) => ({
      name: profile,
      value: profile,
    })),
  });
  ...
  const listRoles = execSync(
    `aws iam list-roles --region ${selectedRegion} --profile ${selectAwsProfile} --output json`,
    { encoding: 'utf8' },
  );
  ...
  if (roles.length === 0) {
    console.log(
      'Tidak ada role yang ditemukan di akun AWS Anda. Silakan buat role dengan izin eksekusi Lambda!',
    );
    return;
  }
  ...
  const awsId = execSync(
    `aws sts get-caller-identity --profile ${selectAwsProfile} --region ${selectedRegion}`,
    { encoding: 'utf8' },
  );
  ...
  fs.writeFileSync(
    path.join(currentDir, 'l.js'),
    `// File ini dibuat secara otomatis oleh L
export const l = {
    awsId: '${awsIdParsed}',
  ...
  console.log(
    `Berhasil menginisialisasi proyek dengan profil AWS: ${selectAwsProfile}`,
  );
}
